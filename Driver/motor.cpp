...
{
	if(pwm > 0){
		GPIO_SetBits(MOTOR_PORT[this->motor], MOTOR_B_PIN[this->motor]);//方向
		GPIO_SetBits(MOTOR_PORT[this->motor], MOTOR_A_PIN[this->motor]);//取消制动
		
	}else if(pwm < 0) {
		GPIO_ResetBits(MOTOR_PORT[this->motor], MOTOR_B_PIN[this->motor]);//方向
		GPIO_SetBits(MOTOR_PORT[this->motor], MOTOR_A_PIN[this->motor]);//取消制动
	}
	else {
		GPIO_ResetBits(MOTOR_PORT[this->motor], MOTOR_A_PIN[this->motor]);//制动
	}
